...
GraphView.prototype.addHTMLEdge = function( particleA, particleB, edgeProperties ) {
	...
	if ( !this['edges'][particleA.id][particleB.id] ) {		
		...
		var edge = document.createElement("div"); //Niko
...
GraphView.prototype.drawHTMLEdge = function (nodeI, nodeJ) {
	...
	var
		Ax = nodeI.positionX*this.skewX + this.centerX,
		Ay = nodeI.positionY*this.skewY + this.centerY,
		Bx = nodeJ.positionX*this.skewX + this.centerX,
		By = nodeJ.positionY*this.skewY + this.centerY,
		lineObjectHandle = this['edges'][nodeI.id][nodeJ.id]['edge'],
		lineImgPath = "http://kylescholz.com/projects/jsviz/0.3.4/layout/view/img/";
	...
	// jsGraphics
	jgGraph = this['edges'][nodeI.id][nodeJ.id]['jg'];
	if (!jgGraph) {
	    jgGraph = new jsGraphics(lineObjectHandle);
	    this['edges'][nodeI.id][nodeJ.id]['jg'] = jgGraph;
	}
	jgGraph.clear();
	jgGraph.setColor("#D8DFEA");
	//jgGraph.setStroke(Stroke.DOTTED);
	jgGraph.drawLine(Ax, Ay, Bx, By);
	jgGraph.paint();
	// End jsGraphics
 ...
 * @param {Particle} particleB
 */
GraphView.prototype.drawHTMLEdgeOld = function ( nodeI, nodeJ ) {
